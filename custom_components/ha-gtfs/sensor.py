...
#    @Throttle(MIN_TIME_BETWEEN_UPDATES)
    def update(self):
        ...
        """ Check if the GTFS files is valid for todays date """
        if gtfs.valid_date(now_d) is False:
            _LOGGER.error("GTFS file is not valid for todays date.")
            empty = []
            self._today_stop_times = pd.DataFrame(empty)
            self._tomorrow_stop_times = pd.DateFram(empty)
            return

        trips = gtfs.day_trips(now_d)
        stop_times = gtfs.stop_times[gtfs.stop_times.trip_id.isin(trips.trip_id) & (gtfs.stop_times.stop_id == self._stop_id) & (gtfs.stop_times.pickup_type != 2)]
        ...
        for index, stop in today_stop_times.iterrows():
            ...
            line = gtfs.routes[gtfs.routes.route_id == trip['route_id']].iloc[0]['route_short_name']
            today_stop_times.loc[[index],['route_short_name']] = line
            temp_td = timedelta(seconds = self._time_to_sec(stop["arrival_time"]))
            today_stop_times.loc[[index],['arrival_dt']] = now_dt.replace(tzinfo=None, hour=0, minute=0, second=0) + temp_td
        ...
        for index, stop in tomorrow_stop_times.iterrows():
            ...
            temp_td = timedelta(seconds = self._time_to_sec(stop["arrival_time"]))
            tomorrow_stop_times.loc[[index],['arrival_dt']] = tomorrow_dt.replace(tzinfo=None, hour=0, minute=0, second=0) + temp_td
    ...
    def get_next(self):
        ...
        next_times = self._today_stop_times[(self._today_stop_times.arrival_time >= nowstr)]
